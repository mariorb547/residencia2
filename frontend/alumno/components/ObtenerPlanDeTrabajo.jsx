...
import { Popover, Table, Row, Popconfirm, message, Button, Col, Icon, Form, Modal, Input, DatePicker, Tooltip, InputNumber, Badge, Switch, Tag } from 'antd';
...
export default class ObtenerPlanDeTrabajo extends Component {
                ...
                <Col xs={{ span: 11, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                  <div>
                    <Popconfirm title="¿Seguro de añadir una nueva tarea?" onConfirm={() => this.addTarea(record.id)}>
                      <Button disabled={this.editableSubactividad(record.id)} key="submit" type="primary" icon="plus">
                ...
                <Col xs={{ span: 5, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                  <div>
                    <Popconfirm title="¿Seguro de editar la subactividad?" onConfirm={() => this.showEditSubactividad(record.id, record.actividad)}>
                      <Button disabled={this.editableSubactividad(record.id)} key="submit" type="primary" icon="edit">
                ...
                <Col xs={{ span: 5, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                  <div>
                    <Popconfirm title="¿Seguro de eliminar?" onConfirm={() => this.deleteSubactividad(record.id, record.id_actividad_general, record.id_orden)}>
                      <Button disabled={this.editableSubactividad(record.id)} key="submit" type="primary" icon="delete" />
              ...
              <Row>
                ...
                <ButtonGroup>
                  <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerSubactividad(record.id, record.id_actividad_general, record.id_orden, "subir")}>
                    <Button disabled={this.editableSubactividad(record.id)} type="primary" icon="up" />
                  </Popconfirm>
                  ...
                  <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerSubactividad(record.id, record.id_actividad_general, record.id_orden, "bajar")}>
                    <Button disabled={this.editableSubactividad(record.id)} type="primary" icon="down" />
                  </Popconfirm>
                </ButtonGroup>
              </Row>
            ) : null
        ...
        render: (text, record) => (
          this.state.dataSource_observaciones.length >= 1
            ? (
              <Row>
                <span>
                  <Badge disabled={this.editableSubactividad(record.id)} count={this.state.dataSource_observaciones.filter(solucion => {
  ...
  editableSubactividad = (id_subactividad) => {
    var respuesta = true;
    this.state.dataSource_tareas.map((tarea) => {
      if (tarea.id_subactividad === id_subactividad && tarea.estado_revision_plan !== "aprobado") {
        respuesta = false;
      }
    })
    return respuesta;
  ...
  editableActividadGeneral = (id_actividad_general) => {
    var respuesta = true;
    var q = this.state.dataSource_subactividades;
    console.log("-------------------------------------------------")
    console.log(q)
    console.log("-------------------------------------------------")
    ...
    try {
      this.state.dataSource_subactividades.map((subactividad) => {
        ...
        if (subactividad.id_actividad_general === id_actividad_general) {
          this.state.dataSource_tareas.map((tarea) => {
            if (tarea.id_subactividad === subactividad.id && tarea.estado_revision_plan !== "aprobado") {
              respuesta = false;
            }
          })
        }
            ...
            <Row>
              ...
              <Col span={12}>
                ...
                <Popconfirm title="¿Seguro de editar la tarea?" onConfirm={() => this.showEditTarea(record.id, record.tarea, record.horas, record.entregable, record.fecha_entrega)}>
                  <Button disabled={record.estado_revision_plan === "aprobado" ? true : false} type="primary" icon="edit" />
                </Popconfirm>
              ...
              <Col span={12}>
                <div>
                  <Popconfirm title="¿Seguro de eliminar?" onConfirm={() => this.deleteTarea(record.id, record.id_subactividad, record.id_orden)}>
                    <Button key="submit" disabled={record.estado_revision_plan === "aprobado" ? true : false} type="primary" icon="delete">
            ...
            <Row>
              <ButtonGroup>
                <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerTarea(record.id, record.id_subactividad, record.id_orden, "subir", record.fecha_entrega)}>
                  <Button disabled={record.estado_revision_plan === "aprobado" ? true : false} type="primary" icon="up" />
                </Popconfirm>
                ...
                <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerTarea(record.id, record.id_subactividad, record.id_orden, "bajar", record.fecha_entrega)}>
                  <Button disabled={record.estado_revision_plan === "aprobado" ? true : false} type="primary" icon="down" />
                </Popconfirm>
              </ButtonGroup>
                ...
                <div>
                  <span>
                    <Badge count={this.state.dataSource_observaciones.filter(solucion => {
                      ...
                      <Button disabled={record.estado_revision_plan === "aprobado" ? true : false} type={"primary"} icon="eye" onClick={() => this.showObservacion(record.id, "plan_de_trabajo")} >Ver</Button>
                    </Badge>
                  </span>
                </div>
              </Col>
...
statusPlanDeTrabajo = () => {
  ...
  axios.get(`/api/plan_de_trabajo/${this.state.proyecto.id}/get_actividad_general`)
    .then(res => {
          ...
          actividad.subactividades.map((subactividad) => {
            subactividad.tareas.map((tarea) => {
              if (tarea.estado_revision_plan === "revision") {
                ...
                tarea.observaciones.map((observacion) => {
                  if (observacion.id_tarea === tarea.id && observacion.tipo_observacion === "plan_de_trabajo" && !observacion.estado) {
                    tareas.push(observacion.observacion + "\n")
                  }
      ...
      if (estado) {
        message.warning('No se puede notificar al asesor interno, se encontraron observaciones sin corregir');
      } else if (tareas.length == 0) {
        message.warning('No se encontraron observaciones');
...
enviarNotificacion = (correo, mensaje) => {
  ...
  axios.post('/api/plan_de_trabajo/notificacion_observaciones_plan', {
    correo: correo,
    mensaje: "Se le notifica que el residente " + proyecto.anteproyecto.alumno.nombre + " " + proyecto.anteproyecto.alumno.ap_paterno + " " + proyecto.anteproyecto.alumno.ap_materno + " ha correguido las observaciones del plan de trabajo de las siguientes tareas: \n" +
      mensaje +
      "\n Se le solicita nuevamente la revisión",
    subject: "Notificación de correcciones"
  }).then((res) => {
    // console.log(res)
    if (res.status === 200) {
...
render() {
            ...
            <Row>
              ...
              <Col xs={{ span: 11, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                <div>
                  ...
                  <Popconfirm title="¿Seguro de añadir una nueva subactividad?" onConfirm={() => this.addSubactividad(record.id)}>
                    <Button disabled={this.editableActividadGeneral(record.id)} key="submit" type="primary" icon="plus" >
              ...
              <Col xs={{ span: 5, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                ...
                <Popconfirm title="¿Seguro de editar actividad general?" onConfirm={() => this.showEditActividadGeneral(record.id, record.actividad, record.objetivo, record.entregable, record.id_proyecto)}>
                  <Button disabled={this.editableActividadGeneral(record.id)} type="primary" icon="edit" />
                </Popconfirm>
              ...
              <Col xs={{ span: 5, offset: 1 }} lg={{ span: 6, offset: 2 }}>
                <div>
                  <Popconfirm title="¿Seguro de eliminar?" onConfirm={() => this.handleDeleteActividadGeneral(record.id, record.id_proyecto, record.id_orden)}>
                    <Button disabled={this.editableActividadGeneral(record.id)} key="submit" type="primary" icon="delete">
            ...
            <Row>
              <ButtonGroup>
                <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerActividadGeneral(record.id_proyecto, record.id, record.id_orden, "subir")}>
                  <Button disabled={this.editableActividadGeneral(record.id)} type="primary" icon="up" />
                </Popconfirm>
                ...
                <Popconfirm title="¿Seguro de recorrer de posición?" onConfirm={() => this.recorrerActividadGeneral(record.id_proyecto, record.id, record.id_orden, "bajar")}>
                  <Button disabled={this.editableActividadGeneral(record.id)} type="primary" icon="down" />
                </Popconfirm>
  ...
  return (
    <div>
      ...
      <FormAddSubactividad visible1={this.state.visibleRegistrarSubactividad} obtenerSubactividades={this.obtenerSubactividades} proyecto={this.state.actividad_general} tipoRegistro={"agregar"} />
